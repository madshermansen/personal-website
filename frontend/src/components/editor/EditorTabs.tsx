    ...
    <div className="flex bg-black/30 border-b border-primary/20 overflow-x-auto">
      {openFiles.map((file) => (
        <div
          ...
          className={`flex items-center gap-2 px-4 py-2 border-r border-primary/10 cursor-pointer transition-all duration-200 font-mono text-sm whitespace-nowrap
            ${activeFile === file.key
              ? 'bg-primary/15 text-primary border-b-2 border-b-primary'
              : 'text-text/70 hover:bg-primary/10 hover:text-accent'}`}
        >
          <span>{file.icon}</span>
          <span>{file.name}</span>
          {onTabClose && (
